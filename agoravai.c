    ...
    while(1){
        ...
        switch(n){
            ...
            case 5:{
                ...
                limpar_casa2(lista, numAlunosporTurma, i);
                p=NULL;
                printf("encerrando...\n");
                break;
            }
...
void gravar_info(int *numAlunosporTurma, Student** arr, int i, float** m){
    ...
    for(aux=0; aux<i; aux++){
        ...
        fprintf(arq, "%s" "%s", arr[aux][t].nome1, arr[aux][t].nome2);
        fprintf(arq, "%d", arr[aux][t].Nusp);
        fprintf(arq, "%f", arr[aux][t].p1);
        fprintf(arq, "%f", arr[aux][t].p2);
        fprintf(arq, "%f", arr[aux][t].trabalho);
...
void limpar_casa2 (Student** Matriz_Alunos, int *numAlunosporTurma, int i){
    for (int aux = 0; aux <numAlunosporTurma[i-1]; aux++){
